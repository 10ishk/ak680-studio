...
- WP17 adds a first controlled setting-write evidence plan only. It does not implement, approve, enable, or execute any setting write.
- WP17 candidate statuses are limited to `insufficient-evidence`, `rejected-too-risky`, `candidate-only`, and `ready-for-future-Red-Team-review`. No status enables execution.
- WP17 risk scoring is conservative from `1` very low to `5` unacceptable; unknown or incomplete write evidence must be treated as high risk.
- WP17 reversibility scoring is conservative from `1` unknown to `5` documented and verifiable; missing rollback evidence keeps a candidate non-executable.
- WP17 requires future evidence for exact observed bytes, report ID, request length, target interface constraints, mutation scope, backup requirements, rollback/recovery, read-back verification, physical verification, hardware risk, and GPL/source cleanliness before a future write proposal can even be reviewed.
- WP17 backup evidence, rollback evidence, read-back evidence, physical verification evidence, candidate completeness, and `ready-for-future-Red-Team-review` status are planning data only and do not unlock write support.
- WP17 validation, classification, import shape checks, and local JSON export are inert data operations. They do not open HID devices, send reports, retry, poll, scan, fuzz, probe, run automatically, or execute hidden follow-up commands.
- WP18 adds first controlled setting-write candidate selection only. It reviews WP17 evidence and records Outcome A or Outcome B without implementing, approving, enabling, or executing write behavior.
- WP18 selects Outcome A for the current WP17 evidence: no candidate is selected for future implementation because the placeholder lighting candidate lacks exact bytes/report ID/request length, low enough risk, high enough reversibility, complete backup/rollback evidence, and complete read-back or physical verification evidence; the keymap candidate is rejected as too risky.
- WP18 Outcome B would require exactly one candidate with risk score `1` or `2`, reversibility score `4` or `5`, hardware-risk classification `visual-only-low-risk` or `single-setting-low-risk`, visual-only or single-setting mutation scope, exact bytes/report ID/request length, backup evidence, rollback evidence, read-back or physical verification evidence, and GPL/source cleanliness.
- WP18 candidate selection records are non-executable. Candidate selection, candidate readiness, evidence completeness, backup evidence, rollback evidence, read-back evidence, and physical verification evidence do not enable execution.
- WP18 validation, review, import shape checks, and local JSON export are inert data operations. They do not open HID devices, send reports, retry, poll, scan, fuzz, probe, run automatically, or execute hidden follow-up commands.
...
- Use the WP17 first-write evidence plan to collect backup, rollback, read-back, physical-verification, and hardware-risk evidence without treating any candidate as executable.
- Use the WP18 candidate-selection review to reject insufficient first-write evidence until exactly one low-risk, reversible, backed-up, verifiable, GPL-clean candidate is ready for a separate future work package and Red Team plan.
- Require a separate future work package and Red Team plan before any first setting-write command can be implemented or approved.
